...
LLM_QUERY_TOOL_DESCRIPTION = dedent(
    """\
    This tool is used to provide general knowledge that can be used to answer the user's query, similar to a Wikipedia-style response. The added knowledge can be used to interpret already existing contexts, or possible answer the user's query directly.

    Only use this tool if you are looking for general knowledge, like a financial concept, an academic formula, a well-known event, historical trends, or other knowledge that is not specific to the user's query or context. If you are looking for specific information, like recent news or company financials, use other tools instead.
