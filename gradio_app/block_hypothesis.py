...
async def execute_hypothesis():
    ...
    continue_research = await hyp_researcher.research()

    output_status = "Research completed." if continue_research else "Research not completed."

    return hyp_researcher.latest_report, hyp_researcher.latest_hypothesis, output_status
...
def create_hypothesis_block():
    with gr.Blocks() as block:
        ...
        gr.Markdown('## Research Question and Lead')
        ...
        with gr.Row():
            with gr.Column():
                gr.Markdown("### Latest Research Report")
                ...
                output_status = gr.Textbox(label="Execution Status", interactive=False)
            with gr.Column():
                @gr.render(inputs=output_hypothesis)
                def show_past_hypothesis(output_hypothesis):
                    global hyp_researcher
                    if hyp_researcher is not None:
                        gr.Markdown("### Past Hypothesis")
                        for lead in hyp_researcher.past_leads:
                            gr.Textbox(
                                lead.hypothesis,
                                show_label=False,
                                max_lines=3,
        ...
        # TODO:
        # 1. one column that shows the newest generated hypothesis, lead, and report
        # 2. one column that lists out all the past hypothesis.

        execute_button.click(execute_hypothesis, outputs=[output_report, output_hypothesis, output_status])
